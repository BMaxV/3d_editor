...
def find_relevant_triangle(point,cell):
    ...
    this didn't really work the way I wanted. it's vestigial code I guess
    """
...
class MyEditor:
    def __init__(self,b):
        ...
        self.test_shadow_support()
    
        
        self.regular_terrain_interaction = True
    ...
    def button_load_terrain(self,*args):
        self.clean_objects()
        self.toggle_regular_terrain_spawning()
        
        if self.regular_terrain_interaction == False:
            self.load_saved_river()
        
    def clean_objects(self):
        
        for key in self.engine_obs:
            ob = self.engine_obs[key]
            ob.removeNode()
        
        for x in self.marker_objects:
            x.removeNode()
    ...
    def toggle_regular_terrain_spawning(self):
        self.regular_terrain_interaction = not self.regular_terrain_interaction
    ...
    def redo_verts(self,d):
        ...
        return all_verts,vert_to_cell
    
    def make_new_verts(self,d,org_verts, vert_to_cell):
        ...
        return new_verts#, my_center
    
    def build_my_edge_list(self, d, index, my_river_tuple_list):
        """
        ...
        flat = True
        my_edge_list = []
        if d[index]["river value"] > 1:
            flat = False
            my_edge_list = []
            for n_index in d[index]["neighbors"]:
                if (index,n_index) not in my_river_tuple_list:
                    continue
                edge_tuple = d[index]["neighbors"][n_index]
                
                neighbor_value = d[n_index]["river value"]
                if neighbor_value >1:
    ...
    def load_saved_river(self):
        ...
        all_verts, vert_to_cell = self.redo_verts(d)
        for index in d:
            
            org_verts = d[index]["points"]
            verts = self.make_new_verts(d, org_verts, vert_to_cell)
            
            my_edge_list,flat = self.build_my_edge_list(d,index,my_river_tuple_list)
            
            if flat:
                faces = [list(range(len(verts)))]
                new_ob = panda_object_create_load.make_object(self.b, verts, faces, twosided=True)
                el = d[index]["elevation"]
                color = colormap(el)
                ...
                if False:
                    # 'house make is my private code that I'm not pushing'
    ...
    def build_buttons(self):
        b1 = pig.create_custom_button("save markers",(-0.6,0,-0.9),self.save,[])
        b2 = pig.create_custom_button("load",(-0.0,0,-0.9),self.load,[])
        b3 = pig.create_custom_button("load terrain",(0.6,0,-0.9),self.button_load_terrain,[])
        b1.setScale(0.05)
        b2.setScale(0.05)
        b3.setScale(0.05)
    ...
    def main(self,inputs,*args):
        if self.regular_terrain_interaction:
            if len(inputs) == 3:
